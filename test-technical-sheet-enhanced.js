...
function validateTechnicalSheetStructure(data) {
  ...
  if (data.preparationSteps) {
    ...
    console.log(
      `✅ ${data.preparationSteps.length} passos de preparo encontrados`,
    );
  }
  ...
  if (data.nutritionalInsights) {
    ...
    console.log(
      `✅ Informações nutricionais completas com ${nutrition.highlights.length} destaques`,
    );
  }
...
function calculateProportionalQuantities(originalData, newServings) {
  console.log(
    `🧮 Calculando quantidades proporcionais para ${newServings} porções...\n`,
  );
  ...
  if (originalData.nutritionalInsights) {
    adjustedNutrition = {
      ...
      calories: (
        parseFloat(originalData.nutritionalInsights.calories) * factor
      ).toFixed(0),
      protein: (
        parseFloat(originalData.nutritionalInsights.protein) * factor
      ).toFixed(1),
      carbs: (
        parseFloat(originalData.nutritionalInsights.carbs) * factor
      ).toFixed(1),
      fat: (parseFloat(originalData.nutritionalInsights.fat) * factor).toFixed(
        1,
      ),
      fiber: (
        parseFloat(originalData.nutritionalInsights.fiber) * factor
      ).toFixed(1),
    };
  }
...
console.log("-" * 30);
const doubledServings = calculateProportionalQuantities(
  mockTechnicalSheetResponse,
  12,
);
...
if (doubledServings.nutritionalInsights) {
  ...
  console.log(
    `   Calorias: ${nutrition.calories} | Proteínas: ${nutrition.protein}g`,
  );
}
...
if (mockTechnicalSheetResponse.preparationSteps) {
  console.log(
    `✅ ${mockTechnicalSheetResponse.preparationSteps.length} passos encontrados:`,
  );
