...
const GRID_COL_SPAN_CLASSES: Record<NonNullable<IComboboxHookFormProps<FieldValues>['colSpan']>, string> = {
...
function ComboboxHookForm<T extends FieldValues>({
    ...
    React.useEffect(() => {
        if (hasService && searchTerm.trim() && filteredOptions.length === 0 && !isLoading && !isFetching) {
        ...
        <Controller
            ...
            render={({ field }) => {
                ...
                const selectedOption = allOptions.find((option) => {
                    const value =
                        'value' in option ? option.value : fieldValue ? String((option as T)[fieldValue]) : '';
                ...
                const displayLabel = buildDisplayText(selectedOption as T | ComboboxOption);
                const buttonLabel = field.value && displayLabel ? displayLabel : placeholder || 'Selecione...';
                const showLoading = hasService && (isLoading || isFetching || isRefetching);
                const showError = hasService && isError;

                return (
                    <Field className={cn('col-span-12 flex flex-col gap-1 p-0', GRID_COL_SPAN_CLASSES[colSpan])}>
                        {label && (
                            ...
                            >
                                <Command>
                                    ...
                                    <CommandList onWheelCapture={(e) => e.stopPropagation()}>
                                        ...
                                        {!showLoading && !showError && filteredOptions.length === 0 && (
                                                ...
                                                const value =
                                                    'value' in option
                                                        ...
                                                        : fieldValue
                                                          ...
                                                          : '';
                                                const labelText = buildDisplayText(option as T | ComboboxOption);
