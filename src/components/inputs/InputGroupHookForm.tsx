...
function InputGroupHookForm<T extends FieldValues>({
    ...
    const renderAddons = (addons: ReactNode | ReactNode[], position: 'start' | 'end') => {
                    ...
                    >
                        ...
                        <InputGroup className={cn(groupClassName)}>
                            ...
                            <InputGroupInput
                                ...
                                className={cn(
                                    hasError ? 'border-2 border-destructive' : showTip ? 'border-2 border-info' : '',
                                    inputClassName
                                )}
                                onFocus={() => {
                                    setIsFocused(true);
                        ...
                        {hasError && (
                            <div className="inline-flex items-center gap-1 max-w-max bg-destructive/90 rounded-sm p-1 text-xs text-white">
                                ...
                                <FieldError className="text-white text-xs" errors={[fieldState.error]} />
                            </div>
                        )}
                    </Field>
                );
