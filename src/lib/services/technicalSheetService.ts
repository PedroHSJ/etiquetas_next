...
export class TechnicalSheetService {
  ...
  static async getTechnicalSheetsByOrganization(
    ...
    page: number = 1,
    limit: number = 10
  ): Promise<{ success: boolean; data?: TechnicalSheet[]; total?: number; error?: string }> {
    try {
  ...
  static async updateTechnicalSheet(
    ...
    try {
      ...
      if (data.tempo_preparo !== undefined) updateData.tempo_preparo = data.tempo_preparo;
   ...
   */
  private static mapDatabaseToTechnicalSheet(data: DatabaseTechnicalSheet): TechnicalSheet {
    return {
      id: data.id,
