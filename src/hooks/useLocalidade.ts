...
export function useLocalidade() {
  ...
  const carregarMunicipiosPorEstado = useCallback(async (estadoId: number) => {
    try {
      setLoading(true)
  ...
  const buscarMunicipiosPorNome = useCallback(async (nome: string, estadoId?: number) => {
    try {
      setLoading(true)
  ...
  // Buscar por CEP
  const buscarCEP = useCallback(async (cep: string): Promise<ViaCEPResponse | null> => {
    try {
      setLoading(true)
  ...
  const buscarOuCriarMunicipioPorCEP = useCallback(async (cep: string) => {
    try {
      setLoading(true)
